...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var targetNode = document.querySelector(
        'div.gr-newsfeed>div.u-clearBoth>div'
    );
    ...
    var callback = function (mutationsList) {
        for (var mutation of mutationsList) {
            if (mutation.type == 'childList') {
                selectAll('img.gr-book__image--large').forEach(
                    (e) => (e.title = e.alt)
                );
                merge_want_to_reads();
            }
...
function get_posts() {
    ...
    let posts = post_elements.map((post_elem, idx) => {
        ...
        book_imgs = selectAll('img.gr-book__image--large', post_elem).map((e) =>
            e.closest('a')
        );
        return {
            idx: idx,
...
function merge_want_to_reads() {
    ...
    unique_users.forEach(
        (u) =>
            (user_to_toread_posts[u] = user_to_posts[u].filter(
                (p) => p.type == 'wants to read'
            ))
    );
    ...
    Object.values(user_to_toread_posts)
        ...
        .forEach((posts) => {
            ...
            base_post_elem.querySelector('.gr-mediaBox .gr-book__title').hidden =
                true;
            base_post_elem.querySelector('.gr-mediaBox .gr-book__author').hidden =
                true;
            base_post_elem.querySelector(
                '.gr-mediaBox .gr-book__additionalContent'
            ).hidden = true;
            ...
            let books_already_merged = new Set(
                base_post.book_imgs.map((img) => img.href)
            );
            let book_imgs_container = base_post_elem.querySelector('.gr-mediaBox');
            posts.slice(1).forEach((post) => {
                post.book_imgs.forEach((img) => {
